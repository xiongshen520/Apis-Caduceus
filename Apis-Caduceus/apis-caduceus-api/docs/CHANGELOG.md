...
- **内部流量伪装升级（INTERNAL_HEADERS）**：发往所有 AI 提供商的请求新增：
  - `Origin: https://replit.com` / `Referer: https://replit.com/` — 请求来源指向 Replit 平台内部
  - `Sec-Fetch-Dest: empty` / `Sec-Fetch-Mode: cors` / `Sec-Fetch-Site: same-site` — 浏览器安全上下文头
  - `Accept: application/json, text/event-stream, */*;q=0.8` / `Accept-Language: en-US,en;q=0.9` — 标准浏览器请求头
  - `User-Agent` 改为 Linux Chrome 124（与 Replit 容器运行环境一致，原为 Windows Chrome）
  - OpenRouter 客户端：`HTTP-Referer` 更新为 `https://replit.com/`，`X-Title` 更新为 `"Replit AI"`
  - Gemini SDK 单独处理：从 INTERNAL_HEADERS 过滤 null 值后传入 `httpOptions.headers`

### Bug 修复

- **多条 system 消息覆盖丢失（关键）**：`convertMessagesForClaude()` 原代码使用 `system = extractText(msg.content)` 赋值，SillyTavern 注入的多条 system 消息（角色卡、世界书条目、作者注释、记忆等）只有最后一条能传递给 Claude，大量上下文材料无声地消失。修复：改为 `system = system ? \`\${system}\n\n\${text}\` : text` 全部拼接。
- **连续同角色消息合并 content 数组情况处理**：`convertMessagesForClaude()` 中合并连续同角色消息时，当 `prev.content` 为数组（非字符串）时原代码进入 `if` 但什么都不做，后续消息被静默丢弃。修复：先提取 `prevText` 再无条件执行合并。
- **Gemini 处理器只读第一条 system 消息**：`handleGemini()` 原代码使用 `.find()` 只取第一条 system 消息，其余世界书条目等全部忽略。修复：改为 `.filter()` 收集所有 system 消息并 `join("\n\n")` 拼接。
